      ...
      >
         ...
         <div>
            <select 
              ...
              :placeholder="$t('select_option')"
              @change="handleRegionChange($event, index)"
            >
...
import { clientIndividualAdd } from '@/rules/client'
...
const handleRegionChange = (e: any, index: number) => {
    ...
    const response = await api.post('client/create', {
      ...
      documents: documents.value.map((item) => {
        return {
          ...
          issue:item.issue? formatDate(item.issue):null,
          expired:item.expired? formatDate(item.expired):null,  
          given: item.given,
        }
      }),
      addresses: addresses.value.map((item) => {
        return {
          ...
          district_code: item.district_code.district_code,
      ...
      works: works.value.map((item) => {
        return {
          ...
          start:item.start? formatDate(item.start):null,
          end: item.end?formatDate(item.end):null,
        }
      }),
      phones: phones.value.map((item) => {
        return {
          phone: '998' + item.phone.replace(/\s+/g, ""),
          type: item.type,
          owner: item.owner,
        }
      })
